...
class Constants(BaseConstants):
    ...
    z = 3
    x = 0.5
    with open('activity2/captcha_names.csv', encoding="utf-8-sig") as f:
...
class Player(BasePlayer):
    ...
    cond1 = models.IntegerField(
        choices = [0,10,20,30,40,50]
    )
    cond2 =  models.IntegerField(
        choices = [0,10,20,30,40,50]
    )
...
class Instructions(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        player.captcha_option = random.choice([1, 2])
        for p in player.in_all_rounds():
            if player.captcha_option == 1:
                p.num_rounds = player.cond1
            elif player.captcha_option == 2:
                p.num_rounds = player.cond2
